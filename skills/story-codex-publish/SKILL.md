---
name: story-codex-publish
description: 为七猫或番茄准备已审查正式章节的离线材料，导出并复核 ZIP，可比较作者提供的草稿副本并留存本地核对摘要。当前不执行平台上传、提交或定时发布。
---

# Story Codex · 发布准备

使用前读取同版 [story-codex](../story-codex/SKILL.md) 的共同约束与目录边界；已在当前上下文则不重复读。共享工具为同级 `story-codex/scripts/story.py`，记其绝对路径为 `<tool>`，所有书籍命令显式传书根 `--book`。
...
1. 用 `status --book "<书根>"` 取得当前 revision 和导出状态，再运行 `template publish` 获得输入结构。由 Codex 填写机器字段，`chapters` 使用连续且升序的数字章号，`mode` 仅支持 `draft`。
...
工具会检查正式版本、对应审查收据、登记路径和导出冲突。原始导入稿的 `imported_unverified` 不是已审查；有效的历史审查可使导入章具备准备资格。遇到阻断时按现有 [审稿修订](../story-codex-review/SKILL.md) 流程在用户授权范围内处理，不伪造通过的审查收据。短篇可以准备指定逐章材料，但这不证明已完本，也不是短故事整篇投稿。
