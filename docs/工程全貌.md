...
| 对象 | 当前状态 |
| --- | --- |
| 开发仓库 | `/Users/cuining/Documents/GitHub/story-skill`，含未提交的开发修改 |
| 本机技能 | `/Users/cuining/.codex/skills/` 下八个同级技能目录；39 个载荷文件已与当前源码逐字节比较 |
| 本地版本号 | `0.6.2`；同号含本地开发改动，不能据此认定等于发布包 |
| GitHub main | 本次在线核对为 `5e49d6ecd53d85b001b002fee9859791e7174ec3`，与本地 HEAD 相同；未提交修改不在远端 |
| 最新公开 Release | `v0.6.2`，标签提交 `d4ddc0e9285b0dff5e100185ed262a985f70ee59`，2026-09-26 16:12:54 北京时间发布 |
| GitHub Packages | 已有 `@ningcui29/story-skill@0.6.2`；本次核对其同步工作流成功，下载字节核验依据已归档发布回执 |

本地相对固定标签已改动七个载荷文件：核心 `SKILL.md`、`story.py`、`story_workbench.py`，规划、写作、审稿三个入口，以及 `drama.md`。主要是工作台候选与恢复、服务管理、章计划提示，以及商业连载规则。当前开发版没有重新发布；按固定标签重新安装会回到该标签内容。

公开 Release ZIP 是 204726 字节，SHA-256 为 `09135f199a66cffd83676a7f7ba29b9de95bd97aef6122eaa33e7b651222e66f`。本日商业规则合并后的本地验证 ZIP 是 221997 字节，SHA-256 为 `374c8e020d7f67cb0dc017c20e32a98003748ea6e772712438fe64d8a32a9684`；二者同号但不同内容。本地临时 ZIP 不是公开发行物。

依据：[Release](https://github.com/NingCui29/story-skill/releases/tag/v0.6.2)、[main CI](https://github.com/NingCui29/story-skill/actions/runs/36229295865)、[Packages 工作流](https://github.com/NingCui29/story-skill/actions/runs/36229108708)、[当版发布及附件核验记录](releases/v0.6.2.md)。本次查了公开 API 的状态和附件摘要，未重新下载远端全部附件。
...
根目录 `scripts/` 有 12 个维护脚本，覆盖打包、项目安装、发布核验、Packages 包装/同步、冒烟、中文稿件回放、超长篇回放、容量、基准、迁移及升级探测。`tests/` 当前 63 个测试模块。`benchmarks/` 保留容量/用量与文学试用材料，`examples/` 保存演示和验收输入，`dist/` 是构建产物；真实小说不存进技能源码或安装目录。工程采用 MIT 许可证，具体以 [LICENSE](../LICENSE) 为准。
...
前三章试用绑定合并商业规则之前的源码，不能用它证明本次商业规则有效。模型评阅不是人类反馈，容量测试不是百万字小说质量证据，平台标签核对不是推荐或审核通过。工作台历史修复有隔离浏览器实测，但尚无本轮真实 Windows 桌面人工验收。
