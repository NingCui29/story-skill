...
main 当前是尚未发布的 **v0.6.1 候选源码**，8 个技能共有 39 个载荷文件；新增 `story_workbench.py`，用于按书生成本地只读快照和静态工作台。它尚无固定标签、Release 或 Packages，也不能证明本机安装已经更新。[工作台实现与限制](本地工作台分析.md)
...
本文使用的入口命名自 v0.6.0 起生效；当前 main 源码路径对应尚未发布的 v0.6.1 候选，v0.5.11 固定标签不提供这些新名称入口。v0.6.0 的远端分发和 Git 方法隔离安装已经核对；v0.6.1 仍只采用本地候选证据。正式版按 [INSTALL.md](../INSTALL.md) 核对实际标签与附件；各平台验证范围以当版记录为准。升级前保留完整书库和本地技能修改，仓库发布不会自动更新本机安装。
...
| 本地只读工作台 | [实现状态与命令](本地工作台分析.md)：显示正式进度、章节页和本地摘要；不写书稿、不联网、不执行平台操作 |
