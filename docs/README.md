...
当前版本：**v0.5.9**。独立代理协作明确交接与复核边界，正文指导增加自然分段判断；沿用 7 个技能入口、34 文件载荷与 schema 2。[版本说明](releases/v0.5.9.md)汇总变化；工程、固定标签、Release、CI 和 Packages 以[本版验证记录](../benchmarks/results/v0.5.9/README.md)的实际结果为准。

使用说明见 [从拆书开始，新起一本书](../README.md#从拆书开始新起一本书)、[大纲细纲怎样保存](中文小说上手.md#大纲细纲怎样保存)、[超长篇实操](超长篇实操.md)和[恢复流程](recovery.md)。v0.5.9 的结果登记到[独立验证目录](../benchmarks/results/v0.5.9/README.md)，不将旧版本通过数改名为新版验收。

本文当前用法对应 v0.5.9；实际默认安装按 [INSTALL.md](../INSTALL.md) 核对固定标签与附件。Windows 的既有 WinError 32 未修复，继续使用 v0.4.0；已经用新版写入的书先完整备份，不直接降级。仓库发布不会自动更新本机安装。
...
v0.5.9 的工程验收、平台结果和分发文件以 [版本说明](releases/v0.5.9.md) 与 [本版验证目录](../benchmarks/results/v0.5.9/README.md) 为准。作品分析的程序核验与文学评阅分别报告，不用测试通过替代解释是否成立。
