...
**当前发布版 v0.6.0 包含 8 个技能、38 个载荷文件。** main 是尚未发布的 v0.6.1 候选源码，共 39 个载荷文件并增加本地只读工作台；它尚无固定标签、Release 或 Packages，也不表示本机安装已更新。v0.5.10 已修复 Windows 正文和报告导出的 WinError 32，并通过当版原生 CI；[当版记录](../benchmarks/results/v0.5.10/README.md)不能代替后续版本的跨平台结果，也不推定覆盖所有设备和文件系统。恢复旧版留下的待导出内容前，先完整备份书库，按 [INSTALL.md](../INSTALL.md) 升级同版整套技能，再核对 `status` 和导出回执；不要盲目降级或覆盖待恢复内容。
...
main 的 v0.6.1 候选才包含工作台。先用实际安装副本的 `story.py --version` 和文件清单核对是否为 0.6.1 候选且含 `story_workbench.py`；只更新仓库源码不会更新安装副本。稳定版 v0.6.0 没有这些命令，不要把“命令不存在”误判为书库损坏。
