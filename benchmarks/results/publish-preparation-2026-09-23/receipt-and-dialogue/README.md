...
2026-09-23，Story Codex v0.5.11 开发源码。本轮为离线章节材料包增加同名包外回执、列表找回和按回执复核，并把“场景对白每轮发言独立成行、换说话人另起段”写入写作与审稿规范。正文格式仍由通读判断，不声称 `lint` 能辨认对白与一般引语。
...
- `story-codex`、`story-codex-write`、`story-codex-review` 与 `story-codex-publish` 的技能结构验证均通过，结果见[技能验证](skill-validation.json)；[文档链接](links.json)和差异格式检查通过。
