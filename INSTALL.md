...
**当前发布版：v0.6.3。** 固定标签 `v0.6.3` 指向 `03dd637ea03f62560b89e5f9030267b06c041e56`。八技能、39 文件；Release 附件和 Packages 的核验结果见[发布记录](docs/releases/v0.6.3.md)。
...
v0.6.3 完整套件 一次安装以下八个同级目录，共 39 个载荷文件：
...
新版 ZIP 名为 `story-skill-0.6.3.zip`，包名为 `@ningcui29/story-skill`。Release 附件和 GitHub Packages 的新包均已公开并完成回下载核验；npm 包不是直接可发现的技能安装。历史文档经过名称遮盖；旧版资源的真实文件名不会因文档改写而改变。

远程安装时，先用官方安装脚本从 `v0.6.3` 固定标签将八个 `--path` 安装到隔离临时目录；另从同一版本 Release 下载 [ZIP](https://github.com/NingCui29/story-skill/releases/download/v0.6.3/story-skill-0.6.3.zip) 和 [校验文件](https://github.com/NingCui29/story-skill/releases/download/v0.6.3/story-skill-0.6.3.zip.sha256)，核对摘要，并逐文件比较临时目录与 ZIP。套件 ZIP 为 232,960 字节，SHA-256 为 `8bbe8ebbf715cb7dcc7e0a4c8b840d0bdd9a2091fdb817461c509c62ace6ea8c`；公开回下载与此一致。ZIP 成员只能是上述八个目录内的普通文件，拒绝绝对路径、越界路径和链接。固定标签、临时安装或附件任一环节不一致，停止升级。

## 从本仓库源码安装

使用已核验的 v0.6.3 固定标签。main 可能包含后续更改，不能仅凭版本号判定其与附件相同。项目内托管安装可在仓库根目录运行：
...
比较目标的八个目录与已验证的新套件，核对文件集合和字节；安装器回执、`__pycache__` 与 `.pyc` 可按各自规则排除。用新版核心 `scripts/story.py` 在隔离书目录执行 `--version`、`--help`、`init`、`status`。只有版本为 `0.6.3`、八目录及全部 39 个载荷文件完整且四个命令通过，才报告安装成功。若安装中断或目标被其他进程修改，保留备份与现场，不把部分结果报告为完成。
